...
interface RoomChatListProps {
  ...
  setSelectedRoomAction: Dispatch<SetStateAction<number | null>>;
  ...
  return (
    <div className="flex flex-col space-y-4 rounded-lg shadow-lg">
      ...
      <ul className="space-y-1">
        {rooms.map((room) => (
          <li key={room.id} onClick={() => setSelectedRoomAction(room.id)} className={`${selectedRoom === room.id ? `bg-highlight-med` : `bg-overlay`} flex flex-row bg-opacity-50 space-x-2 rounded-lg items-center h-12 hover:bg-highlight-med`}>
            <div className="w-7 h-7 ml-2 bg-overlay rounded-full flex items-center justify-center">
              <span className="text-text font-medium">{room.roomName[0]}</span>
            </div>
            <span className="text-text">{room.roomName}</span>
          </li>
        ))}
      </ul>
    </div>
